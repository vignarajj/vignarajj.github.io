...
class MediumRepository {
  ...
  Future<List<MediumPost>> fetchMediumPosts(String username) async {
    ...
    for (final proxy in proxies) {
      ...
      for (int attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          print(
              'Response status: ${response.statusCode}, Data type: ${response.data.runtimeType}');

          if (response.statusCode == 200 && response.data is String) {
            return _parseXmlResponse(response.data as String);
          } else {
            throw Exception(
                'Invalid response: Status ${response.statusCode}, Data ${response.data.runtimeType}');
          }
        } on DioException catch (e) {
          lastError = e;
          print(
              'DioException (Attempt $attempt, Proxy $proxy): ${e.message}, Type: ${e.type}, Response: ${e.response}, Stack: ${e.stackTrace}');
          if (attempt < 3) {
          ...
          print(
              'Unexpected error (Proxy $proxy): $e, Stack: ${StackTrace.current}');
    ...
    throw Exception(
        'Failed to fetch Medium posts after retries: ${lastError?.message}');
  }
  ...
  List<MediumPost> _parseXmlResponse(String xmlString) {
    try {
      print(
          'Parsing XML response: ${xmlString.substring(0, min(xmlString.length, 200))}...');
